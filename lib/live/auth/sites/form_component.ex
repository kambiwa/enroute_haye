defmodule EnrouteHayeWeb.Sites.FormComponent do
  use EnrouteHayeWeb, :live_component

  alias EnrouteHaye.Context.CxtSite

  @impl true
  def update(%{site: sites} = assigns, socket) do
    changeset = CxtSite.change_site(sites)
      ...
      <div style="background: #8B1A1A; border-radius: 0.75rem 0.75rem 0 0;
        ...
        <div>
          ...
          <p style="font-size: 0.72rem; color: rgba(255,255,255,0.65); margin: 0;">
            Fill in the details below to {if @action == :new, do: "create a new site item", else: "update the site item"}.
          </p>
        </div>
      </div>
      ...
      >
            ...
            <div>
              ...
              <div style="display: grid; grid-template-columns: 1fr; gap: 0.75rem;">
                ...
                <.input
                  ...
                  options={["national heritage", "preserved", "archived"]}
                  field={@form[:category]}
                  label="Category"
                />
        ...
        <div style="margin-top: 1.5rem; padding-top: 1rem; border-top: 1px solid #E8E2D9;
          ...
          >
            ...
            {if @action == :new, do: "Create Site", else: "Save Changes"}
          </button>
        </div>
      </.form>
    </div>
  ...
  def handle_event("save", %{"site" => params}, socket) do
    ...
    case socket.assigns.action do
      :new -> create_site(socket, params)
      :edit -> update_site(socket, params)
    end
  end

  defp create_site(socket, params) do
    case CxtSite.create_site(params) do
      {:ok, site} ->
  ...
  defp update_site(socket, params) do
    case CxtSite.update_site(socket.assigns.site, params) do
      {:ok, site} ->
        notify_parent({:saved, site})
