defmodule EnrouteHayeWeb.Auth.Foods.Index do
  ...
  defp apply_action(socket, :edit, %{"id" => id}) do
    ...
    |> assign(:food, CxtFood.get_food!(id))
  end
  ...
  defp apply_action(socket, :index, _params) do
    ...
    |> assign(:page_title, "Foods")
    |> assign(:food, nil)
    |> fetch_filtered_foods()
  end

  @impl true
  def handle_info(
        {EnrouteHayeWeb.Foods.FormComponent, {:saved, _food}},
        socket
      ) do
    {:noreply, fetch_filtered_foods(socket)}
  end

  @impl true
  def handle_event("add_food", _params, socket) do
    {:noreply,
     socket
     |> assign(:page_title, "New Food")
     |> assign(:food, %Food{})
  ...
  defp fetch_filtered_foods(socket) do
    filters = %{
  ...
  def render(assigns) do
    ~H"""
    <Layouts.admin_app flash={@flash} current_scope={@current_scope} current_page={:foods}>
      <div style="padding: 1.5rem; display: flex; flex-direction: column; gap: 1.5rem;">
        ...
        <div style="display: flex; align-items: center; justify-content: space-between;">
          <div>
            ...
            <p style="font-size: 0.82rem; color: #9ca3af;">
              Manage food listings and their details.
            </p>
          </div>
          ...
          <div style="display: flex; align-items: center; gap: 0.75rem;">
            ...
            >
              <.icon name="hero-plus" class="size-4" /> New Food
            </button>
          </div>
        </div>
        ...
        <div style="background: #fff; border: 1px solid #E8E2D9; border-radius: 0.75rem; overflow: hidden;">
          ...
          <div style="padding: 1rem 1.25rem; border-bottom: 1px solid #E8E2D9;">
            <h2 style="font-size: 0.85rem; font-weight: 600; color: #374151;">All Foods</h2>
          </div>

          <Table.table id="tbl_foods" rows={@data}>
            <:col :let={food} label="Name">
              <span style="color: #374151; font-weight: 500;">{food.name}</span>
            </:col>
            <:col :let={food} label="Description">
              <span style="color: #6b7280;">{food.description}</span>
            </:col>
            <:col :let={food} label="Price">
              <span style="color: #6b7280;">{food.price}</span>
            </:col>
        ...
        <%= if @live_action in [:new, :edit] do %>
          <.modal
            id={"crud-food-modal-#{(@food && @food.id) || :new}"}
            show
            on_cancel={JS.push("close")}
          >
            <.live_component
              module={EnrouteHayeWeb.Foods.FormComponent}
              id={(@food && @food.id) || :new}
              title={@page_title}
              action={@live_action}
              food={@food}
